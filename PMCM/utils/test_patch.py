...
def var_all_case(model, test_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    # if dataset_name == "LA":   
    ...
    #     with open('/home/jwsu/semi/data/Pancreas/test.list', 'r') as f:
    #         image_list = f.readlines()
    #     image_list = [
    #         "/home/jwsu/semi/data/Pancreas/Pancreas_h5/" + item.replace('\n', '') + "_norm.h5" for item in image_list
    #     ]
    ...
    for index, image_path in enumerate(image_list):
        if dataset_name == 'LA':                             #todo change
        ...
        # image = h5f['image'][:]
        # label = h5f['label'][:]
        prediction, score_map = test_single_case_first_output(model,
                                                              image,
                                                              stride_xy,
                                                              stride_z,
                                                              patch_size,
...
def test_all_case(args, 
    ...
    for index, image_path in enumerate(loader):
        index = f'{index}_{args.labelnum}'
        if dataset_name == 'LA':                             #todo change
            id = image_path.split('/')[-1]
            h5f = h5py.File(image_path, 'r')
            image = h5f['image'][:]
        ...
        if metric_detail:
            ...
            if num_outputs > 1:
                print('%02d,\t%.5f, %.5f, %.5f, %.5f' % (ith, single_metric_average[0], single_metric_average[1],
                                                         single_metric_average[2], single_metric_average[3]))

        dice_metric.append(single_metric[0])
        jac_metric.append(single_metric[1])
        hd_metric.append(single_metric[2])
        asd_metric.append(single_metric[3])
        ...
        if save_result:
            nib.save(nib.Nifti1Image(prediction.astype(np.float32), np.eye(4)), test_save_path + index + "_pred.nii.gz")
            nib.save(nib.Nifti1Image(image[:].astype(np.float32), np.eye(4)), test_save_path + index + "_img.nii.gz")
            nib.save(nib.Nifti1Image(label[:].astype(np.float32), np.eye(4)), test_save_path + index + "_gt.nii.gz")
    ...
    dice_metric = np.asarray(dice_metric)
    jac_metric = np.asarray(jac_metric)
    hd_metric = np.asarray(hd_metric)
    asd_metric = np.asarray(asd_metric)
    dice_std = np.std(dice_metric)
    jac_std = np.std(jac_metric)
    hd_std = np.std(hd_metric)
    asd_std = np.std(asd_metric)
    std_metric = [dice_std, jac_std, hd_std, asd_std]
    std_metric = np.asarray(std_metric)
    ...
    with open(test_save_path + '../{}_performance_2.txt'.format(model_name), 'w') as f:
