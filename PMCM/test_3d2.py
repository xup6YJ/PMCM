...
parser.add_argument('--model_name', type = str, default='MR_LA_16_15000_0.5_prot_1_org_all_ind', help='model name')
...
os.environ['CUDA_VISIBLE_DEVICES'] = FLAGS.gpu
snapshot_path = "../" + FLAGS.model_name +  '/'  # todo change test flod
...
if FLAGS.dataset_name == 'LA':
    ...
    with open(FLAGS.root_path + list_name, 'r') as f:                                         # todo change test flod
        image_list = f.readlines()
    image_list = [FLAGS.root_path +'/Pancreas_h5/' + item.replace('\n', '') + '_norm.h5' for item in image_list]         # todo change test flod
...
def test_calculate_metric():
    ...
    if FLAGS.dataset_name == "LA":
        avg_metric, std_metric = test_all_case(FLAGS,
                                               FLAGS.model,
                                                1,
                                                net,
                                                image_list,
...
if __name__ == '__main__':
    metric, std_metric = test_calculate_metric()
    print(metric)
    ...
    # result_df = pd.DataFrame(columns=['Dice', 'Jaccard', 'HD', 'ASD' ])
    # result_df.CV = pd.Series(int(FLAGS.cv))
    result_df = pd.DataFrame(columns=[ 'Dice', 'Jaccard', 'HD', 'ASD' , "Dice_std" , "Jaccard_std", "HD_std", "ASD_std"])
    result_df.Dice_std = pd.Series(std_metric[0])
    result_df.Jaccard_std = pd.Series(std_metric[1])
    result_df.HD_std = pd.Series(std_metric[2])
    result_df.ASD_std = pd.Series(std_metric[3])
