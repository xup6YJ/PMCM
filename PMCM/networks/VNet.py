...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        if self.args.bd_loss:
            self.bout_conv = nn.Conv3d(n_filters, n_classes, kernel_size=1, stride=1, padding=0)
...
class Mine3d_v1(nn.Module):
    ...
    def __init__(self,
        ...
        self.args = args
        self.encoder = Encoder(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual)
        self.decoder1 = Decoder_v1(args, n_channels, n_classes, n_filters, normalization, has_dropout, has_residual, 0)
        self.decoder2 = Decoder_v1(args, n_channels, n_classes, n_filters, normalization, has_dropout, has_residual, 2)
    ...
    def forward(self, input):
        ...
        outputs = {}
        outputs['seg'] = [out1['seg'], out2['seg']]
        outputs['prot'] = [out1['prot'], out2['prot']]

        if self.args.simsiam:
            pp1 = self.predictor(out1['prot'])
            pp2 = self.predictor(out2['prot'])
            outputs['prot_pred'] = [pp1, pp2]

        if self.args.bd_loss:
            outputs['bd'] = [out1['bd'], out2['bd']]
            
        if self.training:
            return outputs
        else:
            return out1['seg'], out2['seg']
